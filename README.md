...
| `EXTERNAL_KEY_URL`             | Optional, endpoint to fetch external API key | `""` |
| `EXTERNAL_KEY_SERVICE_TOKEN`   | Optional, service token for the external key service | `""` |
| `EXTERNAL_KEY_JWT_SECRET`      | Optional, JWT secret used to decode the external key | `""` |
| `EXTERNAL_KEY_REFRESH_INTERVAL_HOURS` | Optional, interval to refresh the external key automatically (hours) | `1` |
